...
(library
 ...
 (export new labeled-graph? order directed? nr-of-edges
         ...
         adjacent? 
         next/prev-node next/prev-edge-label next/prev-node-label
...
; ===============================================================
; made by Robin Van Craenenbroek

 ; prev/next: + for next and - for previous node
 (define (next/prev-node graph prev from next/prev)  ; we assume we use cyclic graphs
   (define max (order graph))
   (let iter-search ((to (mod (next/prev from 1) max)))
     (cond ((adjacent? graph from to) (if (eq? prev to)
                                          (iter-search (mod (next/prev to 1) max))
                                          to))
           (else (iter-search (mod (next/prev to 1) max))))))

  (define (next/prev-node-label graph prev from next/prev)
   (label graph (next/prev-node graph prev from next/prev)))
 
 (define (next/prev-edge-label graph prev from next/prev)
   (edge-label graph from (next/prev-node graph prev from next/prev)))
