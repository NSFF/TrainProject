...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; written by Robin Van Craenenbroek

(define (add-train lid n1 n2)
  (set! model (rwm (cons (mk-loco lid n1 n2) (rwm-ls model))
                   (rwm-ns model)
                   (rwm-ss model)
                   (rwm-ts model)
                   (rwm-ds model))))

; note: I used filter (which is O(n), where n is the list-length) on purpose
;       because it makes the code more readable and the simulator will be gone later phases
;       of this project. On top of that, the amount of trains and the amount of times we would use
;       this function is considerably small and can be neglected for efficiency.
(define (remove-train lid)
  (set! model (rwm (filter (lambda(id) (not (eq? (loco-id id) lid)))  (rwm-ls model))
                   (rwm-ns model)
                   (rwm-ss model)
                   (rwm-ts model)
                   (rwm-ds model))))
...
(define (start)
  ...
  (set! running? #t)
  (set! model (load-rwm "Simulator/setup_loop_and_switches.txt"))
  (set! current-thread
        (thread simul-loop)))
